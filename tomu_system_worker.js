...
async function handleRequest(request) {
  ...
  // =========================================================
  // POST /mystic/maya-calendar — マヤ暦診断
  // =========================================================
  if (url.pathname === "/mystic/maya-calendar") {
    var mayaBirthdate = body.birthdate;
    var mayaKin = body.kin;
    var mayaSign = body.sign;
    var mayaTone = body.tone;
    var mayaEmail = body.email;

    if (!mayaBirthdate || !mayaKin || !mayaSign || !mayaTone) {
      return jsonRes({ error: "birthdate, kin, sign, tone are required" }, 400);
    }

    var mayaCheck = await checkPlanAndCount(mayaEmail, "light");
    if (!mayaCheck.ok) {
      return jsonRes({ error: mayaCheck.error, limit: mayaCheck.limit }, mayaCheck.status);
    }

    try {
      var mayaRes = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-api-key": ANTHROPIC_API_KEY,
        ...
        body: JSON.stringify({
          model: "claude-haiku-4-5-20251001",
          max_tokens: 400,
          system: "あなたはマヤ暦占い師です。ツォルキン暦のKIN番号・守護サイン・音をもとに、その人の本質・才能・使命・今日のメッセージを霊的かつ温かみのある言葉で伝えてください。200〜250文字でまとめてください。",
          messages: [{ role: "user", content: "生年月日：" + mayaBirthdate + "、KIN：" + mayaKin + "、守護サイン：" + mayaSign + "、" + mayaTone + "の音" }],
        }),
      });
      if (!mayaRes.ok) {
        return jsonRes({ error: "Anthropic API error", detail: await mayaRes.text() }, mayaRes.status);
      }
      var mayaData = await mayaRes.json();
      var mayaText = (mayaData.content && mayaData.content[0]) ? mayaData.content[0].text : "";
      return jsonRes({ result: mayaText });
    } catch (err) {
      return jsonRes({ error: "Worker error", detail: err.message }, 500);
    }
  }
  ...
  // =========================================================
  // POST /mystic/animal-fortune — 動物占い
  // =========================================================
  if (url.pathname === "/mystic/animal-fortune") {
    var afBirthdate = body.birthdate;
    var afGender = body.gender;
    var afAnimal = body.animal;
    var afNum = body.num;
    var afEmail = body.email;

    if (!afBirthdate || !afGender || !afAnimal || !afNum) {
      return jsonRes({ error: "birthdate, gender, animal, num are required" }, 400);
    }

    var afCheck = await checkPlanAndCount(afEmail, "light");
    if (!afCheck.ok) {
      return jsonRes({ error: afCheck.error, limit: afCheck.limit }, afCheck.status);
    }

    try {
      var afRes = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-api-key": ANTHROPIC_API_KEY,
        ...
        body: JSON.stringify({
          model: "claude-haiku-4-5-20251001",
          max_tokens: 400,
          system: "あなたは弦本將裕式動物占いの占い師です。動物キャラのタイプと性別から、その人の性格の特徴・恋愛傾向・仕事運を温かく具体的に伝えてください。200〜250文字でまとめてください。",
          messages: [{ role: "user", content: afGender + "の" + afAnimal + "（キャラ番号" + afNum + "）タイプ。生年月日：" + afBirthdate }],
        }),
      });
      if (!afRes.ok) {
        return jsonRes({ error: "Anthropic API error", detail: await afRes.text() }, afRes.status);
      }
      var afData = await afRes.json();
      var afText = (afData.content && afData.content[0]) ? afData.content[0].text : "";
      return jsonRes({ result: afText });
    } catch (err) {
      return jsonRes({ error: "Worker error", detail: err.message }, 500);
    }
  }
